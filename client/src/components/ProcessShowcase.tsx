...
export default function ProcessShowcase() {
  ...
  // Create slow-motion scroll transforms for the showcase effect
  const slowMotionY = useTransform(scrollYProgress, [0, 1], [0, -150]);
  const slowMotionScale = useTransform(scrollYProgress, [0, 1], [1, 0.95]);
  const smoothSlowY = useSpring(slowMotionY, { stiffness: 100, damping: 30 });
  const smoothSlowScale = useSpring(slowMotionScale, { stiffness: 100, damping: 30 });

  return (
    <section ref={containerRef} className="min-h-[200vh] bg-slate-900 overflow-hidden">
      {/* Sticky wrapper for slow-motion effect */}
      <div className="sticky top-0 py-32">
        <motion.div 
          style={{ y: smoothSlowY, scale: smoothSlowScale }}
